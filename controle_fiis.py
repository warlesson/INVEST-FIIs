...
class ControleFIIsApp:
    ...
    def construir_interface(self):
        # Formulário para adicionar aporte - reorganizado em linha horizontal
        ...
        tab_rendimentos = ft.Column([
            ...
            ft.Divider(),
            # Cards de resumo organizados em linha
            ft.Row([
    ...
    def salvar_excel(self):
        try:
            ...
            df_rendimentos = pd.DataFrame(columns=["FII", "QTDE COTAS", "ULTIMO PREÇO", "PROVENTOS", "RENDIMENTO MÊS", "RENDIMENTO ANO APROXIMADO", "DATA COM", "DATA QUE FOI GERADA"])
            if self.aportes:
                ...
                # Novo cálculo: rendimento ao mês = qtde cotas x proventos
                df_merged["rendimento_mes"] = df_merged["qtd_total"] * df_merged["provento_total"]
                # Novo cálculo: rendimento ano aproximado = rendimento ao mês x 12
                df_merged["rendimento_ano_aproximado"] = df_merged["rendimento_mes"] * 12
                df_merged["data_gerada"] = datetime.now().strftime("%d/%m/%Y")

                df_rendimentos = df_merged[["fundo", "qtd_total", "preco_medio", "provento_total", "rendimento_mes", "rendimento_ano_aproximado", "data_com", "data_gerada"]]
                df_rendimentos.columns = ["FII", "QTDE COTAS", "ULTIMO PREÇO", "PROVENTOS", "RENDIMENTO MÊS", "RENDIMENTO ANO APROXIMADO", "DATA COM", "DATA QUE FOI GERADA"]
    ...
    def atualizar_tabelas(self):
        ...
        if self.aportes:
            ...
            total_rendimento_mes = df_merged["rendimento_mes"].sum()
            total_rendimento_ano = df_merged["rendimento_ano_aproximado"].sum()
            ...
            for _, row in df_merged.iterrows():
                self.tabela_rendimentos.rows.append(ft.DataRow(cells=[
                    ...
                    ft.DataCell(ft.Text(f'R$ {row["rendimento_ano_aproximado"]:.2f}')),
